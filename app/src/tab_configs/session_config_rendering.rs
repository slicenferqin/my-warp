...
fn localized_session_type_pill_label(session_type: &SessionType) -> String {
    session_type.pill_label()
}
    ...
    F: Fn(usize, &mut EventContext, Vector2F) + 'static,
{
    ...
    let label = Text::new_inline(
        warp_i18n::tr("app-onboarding-tab-config-session-type"),
    ...
    for (i, session_type) in session_types.iter().enumerate() {
        ...
        let name = Text::new_inline(
            localized_session_type_pill_label(session_type),
            appearance.ui_font_family(),
            14.,
        )
    ...
    F: Fn(&mut EventContext, Vector2F) + 'static,
{
    ...
    let label = Text::new_inline(
        warp_i18n::tr("app-onboarding-tab-config-select-directory"),
        appearance.ui_font_family(),
        12.,
    )
    .with_color(if on_accent_bg {
    ...
    F: Fn(&mut warpui::EventContext, warpui::geometry::vector::Vector2F) + 'static,
{
        ...
        Hoverable::new(tooltip_mouse_state, move |state| {
            ...
            if state.is_hovered() {
                let tooltip = Container::new(
                    Text::new_inline(
                        warp_i18n::tr("app-onboarding-tab-config-worktree-git-tooltip"),
                        font_family,
                        12.,
                    )
    ...
    let label = Text::new(
        warp_i18n::tr("app-onboarding-tab-config-auto-create-worktree"),
        appearance.ui_font_family(),
        12.,
    )
    ...
    F: Fn(&mut EventContext, Vector2F) + 'static,
{
    ...
    let checkbox_el = if disabled {
        ...
        Hoverable::new(tooltip_mouse_state, move |state| {
            ...
            if state.is_hovered() {
                let tooltip = Container::new(
                    Text::new_inline(
                        warp_i18n::tr("app-onboarding-tab-config-autogenerate-tooltip"),
                        font_family,
                        12.,
                    )
                    .with_color(theme.background().into_solid())
    ...
    let label = Text::new(
        warp_i18n::tr("app-onboarding-tab-config-autogenerate-branch"),
        appearance.ui_font_family(),
        12.,
    )
    .with_color(label_color)
