...
const DESCRIPTION_TEXT_KEY: &str = "settings-mcp-description";
...
const EMPTY_STATE_TEXT_KEY: &str = "settings-mcp-empty-state";
const NO_SEARCH_RESULTS_TEXT_KEY: &str = "settings-mcp-no-search-results";
...
impl MCPServersListPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        search_editor.update(ctx, |editor, ctx| {
            ...
            editor.set_placeholder_text(warp_i18n::tr("settings-mcp-search-placeholder"), ctx);
        });
        let search_bar = ctx.add_typed_action_view(|_| SearchBar::new(search_editor.clone()));

        let add_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(warp_i18n::tr("settings-mcp-add"), NakedTheme)
                .with_icon(Icon::Plus)
    ...
    fn create_template_server_card(
        ...
        let server_card = ServerCardView::new(
            ...
            template
                .description
                .clone()
                .or_else(|| Some(warp_i18n::tr("settings-code-lsp-available-download"))),
        ...
        match update {
            MCPServerUpdate::CloudTemplate { .. } => {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast =
                        DismissibleToast::success(warp_i18n::tr("settings-mcp-server-updated"));
        ...
        > = std::sync::LazyLock::new(|| {
            vec![
                FormattedTextFragment::plain_text(warp_i18n::tr(
                    "settings-mcp-auto-spawn-description",
                )),
                FormattedTextFragment::hyperlink(
                    warp_i18n::tr("settings-mcp-see-supported-providers"),
    ...
    fn render_page_body(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let description_fragments = vec![
            FormattedTextFragment::plain_text(warp_i18n::tr(DESCRIPTION_TEXT_KEY)),
            FormattedTextFragment::hyperlink(
                warp_i18n::tr("settings-mcp-learn-more"),
                ...
                && filtered_file_based_cards.is_empty()
            {
                ...
                if !owned_server_cards.is_empty() {
                    page.add_child(self.render_server_cards_section(
                        &warp_i18n::tr("settings-mcp-section-my-mcps"),
                        &owned_server_cards,
                        appearance,
                        app,
                    ));
                ...
                if !shared_server_cards.is_empty() {
                    ...
                    let shared_by_text = match team_name {
                        Some(name) => warp_i18n::tr_with_args(
                            "settings-mcp-section-shared-by-warp-and-team",
                            &[("name", name.as_str())],
                        ),
                        None => warp_i18n::tr("settings-mcp-section-shared-by-warp-and-devices"),
                    };
                    ...
                    page.add_child(self.render_server_cards_section(
                        &warp_i18n::tr("settings-mcp-section-shared-from-warp"),
                ...
                for (provider, cards) in &filtered_file_based_cards {
                    ...
                    let section_title = warp_i18n::tr_with_args(
                        "settings-mcp-section-detected-from",
                ...
                Align::new(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .wrappable_text(warp_i18n::tr(EMPTY_STATE_TEXT_KEY), true)
                ...
                Align::new(
                    Flex::column()
                        ...
                        .with_child(
                            appearance
                                .ui_builder()
                                .wrappable_text(warp_i18n::tr(NO_SEARCH_RESULTS_TEXT_KEY), true)
                                .with_style(style::description_text(appearance))
                                .build()
                                .finish(),
                        )
        ...
        for provider in MCPProvider::iter() {
            ...
            let server_card = ServerCardView::new(
                ...
                installation
                    ...
                    .description
                    .clone()
                    .or_else(|| Some(warp_i18n::tr("settings-mcp-detected-from-config-file"))),
    ...
    fn get_title_chip_text(
        ...
        match item_id {
            ...
            | ServerCardItemId::TemplatableMCPInstallation(_) => {
                ...
                if is_shared {
                    match creator {
                        Some(creator) => Some(TitleChip::text(warp_i18n::tr_with_args(
                            "settings-mcp-chip-shared-by",
                            &[("creator", creator.as_str())],
                        ))),
                        None => Some(TitleChip::text(warp_i18n::tr(
                            "settings-mcp-chip-shared-by-team-member",
                        ))),
                    }
                } else if matches!(item_id, ServerCardItemId::TemplatableMCP(_)) {
                    Some(TitleChip::text(warp_i18n::tr(
                        "settings-mcp-chip-from-another-device",
                    )))
                } else {
                    None
                }
