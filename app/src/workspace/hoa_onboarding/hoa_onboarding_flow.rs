...
impl HoaOnboardingFlow {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let cta_button = ctx.add_view(|_ctx| {
            ActionButton::new(
                warp_i18n::tr("app-hoa-see-whats-new"),
        ...
        let next_vtabs_button = ctx.add_view(|ctx| {
            ActionButton::new(warp_i18n::tr("app-hoa-next"), HoaPrimaryButtonTheme)
        ...
        let dismiss_vtabs_button = ctx.add_view(|ctx| {
            ActionButton::new(warp_i18n::tr("app-hoa-dismiss"), HoaPrimaryButtonTheme)
                .with_keybinding(KeystrokeSource::Fixed(enter.clone()), ctx)
                .on_click(|ctx| ctx.dispatch_typed_action(HoaOnboardingAction::Dismiss))
        });
        ...
        let finish_button = ctx.add_view(|ctx| {
            ActionButton::new(warp_i18n::tr("app-hoa-finish"), HoaPrimaryButtonTheme)
    ...
    fn render_vertical_tabs_callout(
        ...
        let checkbox_label = Text::new_inline(
            warp_i18n::tr("app-hoa-switch-back-horizontal-tabs"),
        ...
        self.render_callout_content(
            warp_i18n::tr("app-hoa-vertical-tabs-callout-title"),
            warp_i18n::tr("app-hoa-vertical-tabs-callout-description"),
            Some(checkbox_row),
            button,
            appearance,
        )
    ...
    fn render_inbox_callout(&self, appearance: &Appearance) -> Box<dyn Element> {
        let title = Text::new(
            warp_i18n::tr("app-hoa-inbox-callout-title"),
            appearance.ui_font_family(),
            16.,
        )
        .with_color(callout_title_color(appearance))
        ...
        let learn_more_fragment = FormattedTextFragment {
            text: warp_i18n::tr("app-hoa-learn-more").into(),
        ...
        let formatted = FormattedText::new([FormattedTextLine::Line(vec![
            FormattedTextFragment::plain_text(warp_i18n::tr("app-hoa-inbox-callout-description")),
            learn_more_fragment,
        ])]);
