    ...
    pub fn new(navigation_mode: NavigationMode, ctx: &mut ViewContext<Self>) -> Self {
        ...
        let search_bar = ctx.add_typed_action_view(|ctx| {
            SearchBar::new(
                ...
                warp_i18n::tr("command-palette-search-placeholder"),
        ...
        let placeholder_element = QueryResultRenderer::new(
            MatchedBinding::placeholder(warp_i18n::tr("command-palette-no-results")).into(),
                        ...
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                            toast_stack.add_ephemeral_toast(
                                DismissibleToast::error(warp_i18n::tr(
                                    "command-palette-cannot-switch-conversation-monitoring-command",
                                )),
                                window_id,
                                ctx,
                            );
            ...
            CommandPaletteItemAction::NewConversation => {
                ...
                if can_start_new_conversation {
                    ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                        toast_stack.add_ephemeral_toast(
                            DismissibleToast::error(warp_i18n::tr(
                                "command-palette-cannot-start-conversation-monitoring-command",
                            )),
                            window_id,
                            ctx,
                        );
